...
const imageMap: Record<string, string> = {
  pande: pandeImage,
  'mpanga-kipengele': mikumiImage,
  'tabora-zoo': mikumiImage,
  selous: selousImage,
  'ikorongo-grumet': selousImage,
  'maswa': pandeImage,
  'kijereshi': pandeImage,
  'lukwati-piti': selousImage,
  'moyowosi': mikumiImage,
  'mkungunero': pandeImage,
  'swagaswaga': selousImage,
  'liparamba': mikumiImage,
  'lukwika-lumesule': pandeImage,
};
        ...
        <div className="container-safari">
          ...
          >
            <span className="inline-block px-4 py-1 bg-jungle-yellow/10 text-jungle-yellow rounded-full text-sm font-medium mb-4">
              {t('destinations.page.badge')}
            </span>
            <div className="min-h-[4rem] md:min-h-[5rem] mb-6">
              <Typewriter
                text={t('destinations.page.title')}
                speed={80}
                delay={200}
              ...
              >
                ...
                <div className={`relative ${index % 2 === 1 ? 'lg:col-start-2' : ''}`}>
                  <div className="relative overflow-hidden rounded-2xl group">
                    <img
                      src={imageMap[dest.slug] || pandeImage}
                      alt={dest.name}
                      className="w-full aspect-[4/3] object-cover transition-transform duration-700 group-hover:scale-105"
                      onError={(e) => {
                        const target = e.target as HTMLImageElement;
                        target.src = pandeImage;
                      }}
                    />
                ...
                <div className={index % 2 === 1 ? 'lg:col-start-1' : ''}>
                  <span className="inline-block px-3 py-1 bg-primary/10 text-primary text-xs font-medium rounded-full mb-4">
                    {t('destinations.page.established')} {dest.established}
                  </span>
                  ...
                  <div className="mb-6">
                    <h4 className="text-sm font-semibold text-foreground mb-3">{t('destinations.page.keyWildlife')}</h4>
                  ...
                  >
                    {t('destinations.page.explore')} {dest.name.split(' ')[0]}
